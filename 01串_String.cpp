...
//定义了一个元素类型为char，含有80个元素的数组类型SString
...
/* 若主串S中存在和串T相同的子串，则返回子串T在主串S中第pos个字符之后的位置。若不存在,则函数返回值为0。 */
...
int Index2(SString S, SString T, int pos) 
{
	...
	if (pos > 0) 
	{
		...
		while (i <= n-m+1) 
		{
			SubSString (sub, S, i, m);	/* 取主串中第i个位置长度与T相等的子串给sub */
